...
def main():
    for traj in range(len(data)):
        ...
        for ob in range(len(data[traj]['observations'])):
            ...
            traj_obs.append(np.hstack((env.get_observation()['object'], proprio_state)))
    ...
    print(f"Success! Dumping data to {env_cfg['demo_pkl'][:-4] + '_low_dim.pkl'}")
    pickle.dump(img_data, open(env_cfg['demo_pkl'][:-4] + '_low_dim.pkl', 'wb'))
